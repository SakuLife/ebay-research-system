...
### ⚠️ 「ローカルでは動くのにクラウドだけ落ちる」の正体（2026-07-29）

クラウド実行だけ Gemini検証が全滅し、別商品が仕入先として書き込まれた。
リトライを入れても直らず、ローカルでは同じ入力が正常に判定できた。

**真因: `.env` の `GEMINI_API_KEY` が無効なキーだった。**
ローカルで動いていたのは、**OSの環境変数に別のキーが設定されており、
`load_dotenv()` が既存の環境変数を上書きしないため**、そちらが使われていたから。
GitHub Actionsには環境変数が無いので、Secretの（無効な）キーがそのまま使われて落ちていた。

実測（3つのキーを実際に叩いて確認）:

| キー | 長さ | 結果 |
|---|---|---|
| `.env` に書かれていた値（末尾 tcSAfA） | 39 | ❌ `API key not valid` |
| `.env`の値＋末尾1文字（写し間違い） | 40 | ❌ 同上 |
| **OS環境変数の値（末尾 8YOejA）** | 53 | ✅ 成功 |

**教訓**:
1. **ローカルで動いた＝設定が正しい、ではない。** 環境変数が裏で救っている場合がある。
   キーを検証するときは「実際にそのファイルの値で」叩くこと。
2. 「API key not valid」はリトライで直らない。レート制限と混同しない。
3. `.env` は 2026-07-29 に有効なキーへ差し替え済み。
   親の `PythonSystem/CLAUDE.md` にも同じ地雷（`load_dotenv` は環境変数を上書きしない）が記載されている。

### Geminiのレート制限と実費（2026-07-29 実測）

**レート制限は「プロジェクト単位」であってAPIキー単位ではない**（公式明記:
"Rate limits are applied per project, not per API key"）。
＝ **同じプロジェクトでキーを作り直しても枠は増えない。**

| | 内容 |
|---|---|
| 無料枠 | 完全に$0。**制限に達しても課金されず429エラーが返るだけ** |
| 無料枠の制限 | Flash系で概ね 5〜15 RPM / 1,000 RPD（AI Studioで要確認） |
| 有料化の条件 | 課金アカウントをリンクするとTier1（150〜300 RPM）へ |
| 有料時の単価 | gemini-flash-latest（3.5 Flash相当）: 入力 $1.50/1M・出力 $9.00/1M |

**実測コスト**（90商品の実行・実トークン計測）:
翻訳87回・画像分析96回・同一商品検証88回・画像比較64回で
入力283,954tok / 出力67,038tok ＝ **約154円（1商品あたり約1.7円）**。
月1,000商品なら**約1,700円/月**。

⚠️ 画像比較は1回で入力2,196トークン（画像2枚）を食い、コストの大半を占める。
テキストのみの翻訳は30トークンなので桁が違う。**削るなら画像比較の回数**。

⚠️ **無料枠のままだとレート制限で検証が飛ぶ**（→ 別商品が採用される）。
リトライは実装済みだが、根本的に回したいなら課金アカウントのリンクが要る。

### ⚠️ 相性の悪いキーワードでクレジットが溶ける（`MAX_ITEMS_PER_KEYWORD`）

設定シートの「検索商品数」は**出力する件数の目標**であって、処理する件数の上限ではない。
1件も出力できないキーワードだと目標に到達せず、**最大4ページぶん延々と探索し続ける**。

実測（2026-07-29「Iron man」・UK市場・最低利益¥5,000）:
**90商品を処理して出力0件、35クレジット消費**。
客が効かないキーワードを10個並べれば、それだけで月のクォータが飛ぶ。

対策として `MAX_ITEMS_PER_KEYWORD`（既定40）を超えたら打ち切る。
＝ 1キーワードあたりの最悪コストが上限で読めるようになる。

**「Iron man」のような抽象的・作品名系のキーワードは元々ヒットしない**
（客自身も2026-03-24の分析で同じ結論に達している）。
実際に出力が出るのは消費財系（化粧品・シャンプー・調理器具など型番が明確なもの）。
納品前に「効くキーワード」を用意しないと、客は前回と同じ「0件」の不満を持つ。
