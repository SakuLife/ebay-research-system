...
【判定対象外（これらはOK/NGの判断に使わない）】
- eBayと仕入先のコンディション差（新品/中古/Open Box等）は無視してよい
  例: eBay「Used」で仕入先が「新品」→ 問題なし（同一商品ならOK）
  例: eBay「Open Box」で仕入先が「中古Cランク」→ 問題なし
...
- コンディション（新品/中古）の差は判断しない。同一商品かどうかだけを判断する
