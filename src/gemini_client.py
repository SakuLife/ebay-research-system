...
それでは調査結果を出力してください:'''
    ...
    def validate_source_match(
        ...
        # conditionに応じてコンディション判定ルールを切り替え
        is_new = condition.lower() == "new" if condition else True
        if is_new:
            condition_rule = """【コンディションルール（New出品）】
- eBayがNew出品のため、仕入先も新品であることが必要
- 仕入先が「中古」「USED」「ランクB」「ジャンク」等の場合 → NG
  例: 仕入先に「中古品」「USED」と明記 → NG
  例: PSA/BGS鑑定済みカード → 中古扱いでNG"""
        else:
            condition_rule = """【コンディションルール（Used出品）】
- eBayがUsed出品のため、仕入先は新品でも中古でもOK
  例: eBay「Used」で仕入先が「新品」→ 問題なし
  例: eBay「Used」で仕入先が「中古Cランク」→ 問題なし
- コンディション差は判断対象外。同一商品かどうかだけを判断する"""

        prompt = f'''あなたはeBay転売の仕入れ判断アシスタントです。
以下のeBay商品と国内仕入先の組み合わせが「同一商品か」を判断してください。
判断は厳格に行い、少しでも疑わしい場合はNGとしてください。
...
【チェック観点】
1. 完全に同一の商品か？（最重要 - 非常に厳しく判断）
   - 商品名・キャラクター名が一致している → OK
   - 型番・モデル番号が一致している → OK
   - 固有名詞が違う場合は別商品 → NG（例: eBay「Nika」vs 仕入「ヤマト」は別商品）
   - 同じシリーズでも別キャラ・別デザインは別商品 → NG
   - ブランドのみ一致で商品名が違う → NG

2. 商品タイプ・サイズ・色・スペックが一致しているか？（重要 - 厳しく判断）
   - 同ブランド・同シリーズでも商品タイプが違えばNG
     例: eBay「Large Pouch」vs 仕入「Coin Pouch」→ NG（サイズ違い）
     例: eBay「Backpack」vs 仕入「Shoulder Bag」→ NG（種類違い）
   - 色が明示されていて違う場合はNG
     例: eBay「Black」vs 仕入「オレンジ」→ NG
   - サイズ表記が違う場合はNG
     例: eBay「Size L」vs 仕入「Size M」→ NG
   - 型番の数字が違う場合はNG
     例: eBay「084」vs 仕入「037」→ NG（別モデル）
     例: eBay「Land Cruiser 300」vs 仕入「Land Cruiser 250」→ NG
   - スポーツ用品のスペック差はNG
     例: ロフト角「9.5°」vs「10.5°」→ NG
     例: フレックス「Stiff」vs「SR」→ NG

3. セット数・個数が一致しているか？（重要）
   - eBayが「4個セット」なのに仕入先が「3個セット」→ NG
   - 個数の不一致は明確にNG

4. 購入可能なサイトか？
   - Amazon、楽天、Yahoo、専門店 → OK
   - メーカー公式ストア → OK
   - 不明なサイトでも商品ページなら → 一旦OK

{condition_rule}
- 価格差の大小は判断対象外（利益計算は別途行う）
...
【SUGGESTIONの意味】※迷ったらskipでOK（誤マッチより見逃しの方が安全）
- accept: 同一商品と確信できる（型番・色・サイズ・タイプ・スペック全て一致）
- skip: 別商品の可能性がある、またはコンディションルール違反
- retry: この仕入先は微妙、別を試したい

【重要ルール】
- 商品名の固有名詞（キャラ名、型番等）が違う場合は必ずskip
- 同じブランド・シリーズでも、商品タイプ・色・サイズ・型番・スペックが1つでも違えばskip
- 「ブランド名+キャラ名が同じ」だけでは不十分。具体的な商品モデルまで一致が必要
- 上記【コンディションルール】に従うこと
...
それでは判断してください:'''
    ...
    def analyze_ebay_item_image(
        ...
        # Used条件ではvintage/collector判定を緩くする
        condition_note = ""
        if condition and condition.lower() != "new":
            condition_note = """
【Used条件での注意】
この商品はeBayでUsed/中古として販売されています。
以下はスキップ不要です:
- ヴィンテージ品・年代物 → Used市場では通常の仕入れ対象
- コレクター向け品 → Used市場では取引可能
- 開封済み・箱なし → Used商品なので問題なし
ただし、カード/セット売り/一番くじ/手作り品/キーワード不一致は引き続きスキップしてください。"""

        prompt = f'''あなたはeBay商品の仕入れ可否を判定するアシスタントです。
以下の商品画像とタイトルを見て、日本の通販サイトで仕入れられるかを判定してください。
...
6. 出品者オリジナルセット（custom_set）
   - 出品者が独自に複数商品をまとめたセット
   - 市販のセット商品ではない

7. アニメ制作素材（production）
   - セル画（Animation Cel）
   - 原画、動画、背景画
   - 手描きのアニメ素材

8. ノーブランド服・アパレル（apparel）
   - 有名ブランド名が明記されていない衣類
   - 型番や商品コードがない衣類
   - 検索しても同一商品が見つからない可能性が高い服

9. 海外限定・輸入転売品（import_only）
   - 日本未発売の海外限定商品
   - 「US exclusive」「Disney Parks」「import」等の表記

10. 手作り・カスタム品（handmade）
   - ハンドメイド、手作り品
   - カスタムペイント、改造品
   - 個人制作の一点物

11. 型番なし・検索困難な商品（unsearchable）
    - 型番/商品コードが一切なく、通販サイトで検索困難
    - 商品名が一般的すぎて特定できない（例: 「花瓶」「グラス」のみ）
    - タイトルに固有の識別子がない商品

【重要な判断基準】
「この商品は日本の通販サイト（Amazon・楽天・Yahoo等）で見つけられるか？」
...
TYPE: [card/lottery/set/promo/mismatch/vintage/custom_set/production/apparel/import_only/collector/handmade/unsearchable/figure/toy/other]
