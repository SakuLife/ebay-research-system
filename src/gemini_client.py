...
【チェック観点】※寛容に判断してください
1. 同一商品か？（重要だが厳密すぎなくてOK）
   - ブランド・メーカーが一致 → OK
   - 商品カテゴリ（ドライバー、アイアン等）が一致 → OK
   - モデル名が概ね一致（年式違いはOK）→ OK
   - 細かいスペック（シャフト硬さ、ロフト角等）が少し違っても → 基本OK（後で人が確認）
   - 明らかに別商品（ドライバーとパター等）→ NG
...
【重要】迷った場合は「accept」にして人に判断を委ねる。
厳しくしすぎると全部NGになってしまうので、寛容に。
