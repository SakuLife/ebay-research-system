...
【チェック観点】
1. 同一商品か？（最重要）
   - 型番・品番が完全一致しているか確認すること
   - 特に以下のカテゴリは型番の完全一致が必須:
     * 自動車・バイクパーツ: OEM品番（例: 8FP-84310-00-00）が一致しないとNG
     * 電子機器: 型番・モデル番号が一致しないとNG
     * ゴルフクラブ: モデル名、シャフト、フレックスが全て一致しないとNG
   - 「同じブランドの似た商品」は不可、「完全に同一の商品」のみ可
   - 車種・適合情報が異なる場合はNG（例: YZF R6用とYZF R1用は別商品）
...
【SUGGESTIONの意味】
- accept: この仕入先でOK（型番が完全一致している場合のみ）
- skip: この商品自体をスキップ（仕入れ困難、または全く違う商品）
- retry: この仕入先はNG、別の仕入先を試すべき（型番不一致の場合はこれ）

【重要】型番・品番が異なる場合は必ず「retry」または「skip」にすること。
「同じメーカーの似た商品」で「accept」にしてはいけない。
