...
【チェック観点】
1. 同一商品か？（最重要 - ここは厳しく判断）
   - 商品名・キャラクター名が一致している → OK
   - 商品名に含まれる固有名詞（キャラ名、型番等）が一致 → OK
   - 固有名詞が違う場合は別商品 → NG（例: eBay「Nika」vs 仕入「ヤマト」は別商品）
   - 同じシリーズでも別キャラ・別デザインは別商品 → NG
   - ブランドのみ一致で商品名が違う → NG

2. セット数・個数が一致しているか？（重要）
   - eBayが「4個セット」なのに仕入先が「3個セット」→ NG
   - eBayが「Set of 4」なのに仕入先が「Set of 3」→ NG
   - 個数の不一致は明確にNG（似ていても別商品）
...
【SUGGESTIONの意味】※迷ったらacceptでOK
- accept: 概ね同一商品と判断できる（細部は人が確認）
- skip: 商品カテゴリが全く違う、または中古品
- retry: この仕入先は微妙、別を試したい（ほぼ使わない）

【重要】商品名の固有名詞（キャラ名、型番等）が違う場合は必ずskip。
同じブランド・シリーズでも、商品名が違えば別商品としてNG判定する。
